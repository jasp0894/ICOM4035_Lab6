package positionListLLDirect;
	 ...
	 *
	 */
	public class NodePositionListCopy_1<T> implements PositionList<T> {
		...
		public NodePositionListCopy_1() {
			header = new DNode<T>(null, null, null, this); 
			trailer = new DNode<T>(null, header, null, this);
			header.setNext(trailer); 
			count = 0;
			iterMaker = new PositionListForwardIteratorMaker<>();
		}
		public NodePositionListCopy_1(PositionListIteratorMaker<T> iterMaker){
			this();
		...
		public Iterable<Position<T>> positions() {
			NodePositionListCopy_1<Position<T>> pList = new NodePositionListCopy_1<Position<T>>(); 
			try { 
				if (!this.isEmpty()) { 
				...
				System.out.println("NodePositionListCopy_1 - positions(): Invalid list"); 
				e.printStackTrace(); 
			}
			return pList;
		}
		...
		private static class DNode<T2> implements Position<T2> {
	        ...
	        private NodePositionListCopy_1<T2> myList;     // list it belongs to
	        
	        public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionListCopy_1<T2> list) { 
	        	element = e; 
	        	next = n; 
	        	prev = p; 
	        	myList = list; 
			...
			public void setList(NodePositionListCopy_1<T2> n) { 
				myList = n; 
			}
			public NodePositionListCopy_1<T2> getList() { 
				return myList; 
			}
